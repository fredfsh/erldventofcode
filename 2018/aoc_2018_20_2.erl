...
bfs(Doors) ->
    bfs_impl(0, queue:from_list([{{0, 0}, 0}]), sets:from_list([{0, 0}]), Doors).

-define(DOORS, 1000).

bfs_impl(CountAcc, Q, Visited, Doors) ->
    case queue:out(Q) of
        {empty, _} ->
            CountAcc;
        {{value, {XY, N}}, Q2} ->
            ...
            NewCountAcc = case N >= ?DOORS of
                              true ->
                                  CountAcc + 1;
                              false ->
                                  CountAcc
                          end,
            bfs_impl(NewCountAcc, NewQ, NewVisited, Doors)
    end.
