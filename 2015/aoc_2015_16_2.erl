...
match(Map) ->
    F = fun({"cats" = K, Val}) -> maps:get(K, Map, Val + 1) > Val;
           ({"trees" = K, Val}) -> maps:get(K, Map, Val + 1) > Val;
           ({"pomeranians" = K, Val}) -> maps:get(K, Map, Val - 1) < Val;
           ({"goldfish" = K, Val}) -> maps:get(K, Map, Val - 1) < Val;
           ({Key, Val}) -> maps:get(Key, Map, Val) =:= Val
        end,
    lists:all(F, ?TRAITS).
