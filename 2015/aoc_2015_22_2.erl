...
magic_missle(#state{mana = Mana}) when Mana < 53 ->
    lose;
magic_missle(State) ->
    case apply_player_effects(State) of
...
apply_player_effects(#state{php = 1}) ->
    lose;
apply_player_effects(#state{php = HP} = State) ->
    apply_effects(State#state{php = HP - 1}).

apply_effects(#state{shield = Shield, poison = Poison, recharge = Recharge,
                     mana = Mana, ohp = HP, spend = Spend} = State) ->
