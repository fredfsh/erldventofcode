...
do(Code) ->
    ...
    run(queue:from_list(Processes), MQ).
...
run(Q, MQ) ->
    ...
    case instruction(Process) of
        {?OP_HALT, _} ->
            run(Q2, MQ);
        {?OP_IN, _} = Instruction ->
            Messages = maps:get(ID, MQ, []),
            Inputs = Input ++ Messages,
            Process2 = execute(Instruction, Process#process{input = Inputs}),
            run(queue:in(Process2, Q2), maps:remove(ID, MQ));
        {?OP_OUT, _} = Instruction ->
            case execute(Instruction, Process) of
                #process{output = [Y, _, ?ADDR | _] = Output}
                  when length(Output) rem 3 =:= 0 ->
                    Y;
                #process{output = [Y, X, Addr | _] = Output} = Process2
                  when length(Output) rem 3 =:= 0 ->
                    F = fun(L) -> L ++ [X, Y] end,
                    MQ2 = maps:update_with(Addr, F, [X, Y], MQ),
                    run(queue:in(Process2, Q2), MQ2);
                Process2 ->
                    run(queue:in(Process2, Q2), MQ)
            end;
        Instruction ->
