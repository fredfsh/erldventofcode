...
-record(process, {output = undefined, ip, code, x, y, dx, dy, whites, painted}).

do(X) ->
    run(#process{ip = 0, code = X, x = 0, y = 0, dx = 0, dy = 1,
                 whites = sets:new(), painted = sets:new()}).
...
run(#process{painted = Painted} = Process) ->
...
paint(#process{x = X, y = Y, painted = Painted} = Process) ->
    Process2 = paint_impl(Process),
    NewPainted = sets:add_element({X, Y}, Painted),
    Process2#process{output = undefined, painted = NewPainted}.

paint_impl(#process{output = 0, x = X, y = Y, whites = Whites} = Process) ->
    Process#process{whites = sets:del_element({X, Y}, Whites)};
paint_impl(#process{output = 1, x = X, y = Y, whites = Whites} = Process) ->
    Process#process{whites = sets:add_element({X, Y}, Whites)}.
