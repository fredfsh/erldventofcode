...
-record(process, {input, output, ip = 0, code}).

-define(AMPS, 5).

do(X) ->
    Phases = lists:seq(0, ?AMPS - 1),
    PhaseSeqs = perms(Phases),
    Process = #process{ip = 0, code = X},
    F = fun(PhaseSeq, Acc) ->
                max(Acc, sim(Process, 0, PhaseSeq))
        end,
    lists:foldl(F, sim(Process, 0, Phases), PhaseSeqs).

perms([]) -> [[]];
perms(L) -> [[H | T] || H <- L, T <- perms(L -- [H])].

sim(Process, Input, PhaseSeq) ->
    F = fun(Phase, Acc) ->
                run(Process#process{input = [Phase, Acc]})
        end,
    lists:foldl(F, Input, PhaseSeq).
