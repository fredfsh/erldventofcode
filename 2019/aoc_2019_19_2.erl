...
search(Code) ->
    F = fun(Y) -> searchx(Y, Code) =/= undefined end,
    Y = binary_search(0, ?MAX, F),
    X = searchx(Y, Code),
    {X, Y}.

searchx(Y, Code) ->
    XMin = linear_search(0, fun(X) -> beam(X, Y, Code) end),
    XMax = linear_search(XMin, fun(X) -> not beam(X, Y, Code) end) - 1,
    F = fun(X) ->
                beam(X, Y, Code)
                    andalso beam(X + ?N - 1, Y, Code)
                    andalso beam(X, Y + ?N - 1, Code)
        end,
    linear_search(XMin, XMax, F).

beam(X, Y, Code) ->
    run(#process{input = [X, Y], code = Code}) =:= 1.

binary_search(Left, Right, _) when Right =:= Left + 1 ->
    Right;
binary_search(Left, Right, Pred) ->
    Mid = (Left + Right) div 2,
    case Pred(Mid) of
        true ->
            binary_search(Left, Mid, Pred);
        _ ->
            binary_search(Mid, Right, Pred)
    end.

linear_search(Min, Pred) ->
    linear_search(Min, Min + ?MAX, Pred).

linear_search(Min, Max, Pred) ->
    linear_search_impl(Min, Max, Pred).

linear_search_impl(I, Max, _) when I =:= Max + 1 ->
    undefined;
linear_search_impl(I, Max, Pred) ->
    case Pred(I) of
        true ->
            I;
        false ->
            linear_search_impl(I + 1, Max, Pred)
    end.
