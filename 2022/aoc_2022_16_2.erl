...
fin({Valves, Flowrates, _PipeSets, PipeMaps}) ->
    ...
    dfs(Flowrates, ?MINUTES, ?START, 0, false, Steps).
...
dfs(Flowrates, Minutes, Current, Pressure, Elephant, Steps) ->
    F = fun(Valve, Flowrate, Acc) ->
                case Minutes - maps:get({Current, Valve}, Steps) - 1 of
                    Rem when Rem > 0, Elephant ->
                    ...
                    Rem when Rem > 0 ->
                        max(
                          max(Acc, dfs(maps:remove(Valve, Flowrates),
                                       ...
                                       Elephant,
                                       Steps)),
                          Pressure + memo(Flowrates, Steps));
                    _ when Elephant ->
                        Acc;
                    _ ->
                        max(Acc, Pressure + memo(Flowrates, Steps))
                end
        end,
    maps:fold(F, Pressure, Flowrates).

memo(Flowrates, Steps) ->
    Key = lists:sort(maps:keys(Flowrates)),
    case ets:lookup(?TAB, Key) of
        [] ->
            Res = dfs(Flowrates, ?MINUTES, ?START, 0, true, Steps),
