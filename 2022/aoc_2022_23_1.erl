...
sim(?ROUNDS, Elves) ->
    F = fun({X, Y}, {MinXAcc, MinYAcc, MaxXAcc, MaxYAcc}) ->
                {min(MinXAcc,X), min(Y,MinYAcc), max(MaxXAcc,X), max(Y,MaxYAcc)}
        end,
    {MinX, MinY, MaxX, MaxY} = sets:fold(F, {u, u, -?ROUNDS, -?ROUNDS}, Elves),
    (MaxX - MinX + 1) * (MaxY - MinY + 1) - sets:size(Elves);
sim(Rounds, Elves) ->
