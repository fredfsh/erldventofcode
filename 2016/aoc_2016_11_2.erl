...
input() ->
    ...
    lists:foldl(F, extra(), lists:seq(1, 4)).
...
-define(EXTRAS, [elerium, dilithium]).

extra() ->
    F = fun(Element, {ElementsAcc, InitAcc}) ->
                Map = maps:put({Element, generator}, 1, InitAcc),
                {sets:add_element(Element, ElementsAcc),
                 maps:put({Element, microchip}, 1, Map)}
        end,
    lists:foldl(F, {sets:new(), maps:put(elevator, 1, maps:new())}, ?EXTRAS).
