...
input() ->
    case io:get_line("") of
        ...
        _ ->
            ...
            State1 = lists:foldl(F, array:new(), L1),
            {ok, L2} = io:fread("", " #~c#~c#~c#~c#"),
            H = fun({[X], I}, Acc) ->
                        [Y] = array:get(I, Acc),
                        array:set(I, [Y, X], Acc)
                end,
            State2 = lists:foldl(H, State1, lists:zip(L2, lists:seq(0, 3))),
            io:get_line(""),
            io:get_line(""),
...
target(Doors) ->
    array:to_list(Doors) =:= [[$A, $A], [$B, $B], [$C, $C], [$D, $D]].
